...
st.markdown("**Insights Pipeline**: Browse canonical travel insights (services, tips, logistics, cultural info)")
...
if insights_df.empty:
    ...
    st.info("💡 Run the insights pipeline to extract travel tips, services, and practical information from videos!")
    st.stop()
...
with col2:
    ...
    unique_destinations = destinations.groupby(['country', 'city']).size().reset_index().shape[0]
    st.metric("Unique Destinations", unique_destinations, help="Unique destination coverage")

with col3:
    # Average confidence
    if 'confidence_score' in insights_df.columns:
        avg_confidence = insights_df['confidence_score'].mean()
        st.metric("Avg Confidence", f"{avg_confidence:.2f}", help="Average confidence score (0-1)")
    else:
...
with col4:
    ...
    if 'source_video_ids' in insights_df.columns:
        ...
        st.metric("Source Videos", len(all_video_ids), help="Videos contributing insights")
    else:
...
with col1:
    # Category filter
    if 'category' in insights_df.columns:
        categories = ['All'] + sorted(insights_df['category'].dropna().unique().tolist())
    ...
    if 'destination_type' in insights_df.columns:
        dest_types = ['All'] + sorted(insights_df['destination_type'].dropna().unique().tolist())
...
with col3:
    ...
    if 'mention_count' in insights_df.columns:
        max_mentions = int(insights_df['mention_count'].max()) if insights_df['mention_count'].max() > 0 else 10
...
# Search box
search_query = st.text_input("🔎 Search by content", placeholder="Type to search in insight content...")
...
if search_query:
    if 'content' in filtered_df.columns:
        filtered_df = filtered_df[
            ...
            filtered_df.get('title', pd.Series()).str.contains(search_query, case=False, na=False)
        ]
...
with col1:
    ...
    if 'category' in filtered_df.columns and 'country' in filtered_df.columns:
        ...
        treemap_data['destination'] = treemap_data.apply(
            lambda x: f"{x['city']}, {x['country']}" if pd.notna(x['city']) else (x['country'] if pd.notna(x['country']) else 'Global'),
            axis=1
        )

        # Group by category and destination
        treemap_grouped = treemap_data.groupby(['category', 'destination']).agg({
            'mention_count': 'sum',
...
with col2:
    ...
    if 'category' in filtered_df.columns and 'destination_type' in filtered_df.columns:
        ...
        sunburst_grouped = sunburst_data.groupby(['category', 'destination_type', 'country']).size().reset_index(name='count')
...
with col1:
    ...
    if 'mention_count' in filtered_df.columns and 'content' in filtered_df.columns:
        ...
        top_mentioned['display'] = top_mentioned.apply(
            lambda x: x['title'] if x.get('title') else (x['content'][:50] + '...' if len(x['content']) > 50 else x['content']),
            axis=1
        )
...
with col1:
    # Download as CSV
    csv_data = filtered_df[['insight_id', 'category', 'content', 'title', 'confidence_score', 'mention_count', 'video_count', 'country', 'city']].copy()
...
for idx, insight in filtered_df.iterrows():
    ...
    # Create header
    display_title = title if title else (content[:80] + '...' if len(content) > 80 else content)
    ...
    with st.expander(f"**{category.upper()}** | {display_title}", expanded=False):
        ...
        if source_videos:
            st.caption(f"Source Videos ({len(source_videos)}): {', '.join(source_videos[:5])}{'...' if len(source_videos) > 5 else ''}")
