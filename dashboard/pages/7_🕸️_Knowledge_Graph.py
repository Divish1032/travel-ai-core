...
if entities_df.empty and insights_df.empty:
    ...
    st.info(
        "💡 Process videos through the entity and insights pipelines to build the knowledge graph"
    )
    st.stop()
...
# Build filters dict
filters = {"graph_type": graph_type, "max_nodes": max_nodes}
...
with col4:
    ...
    if nodes and edges:
        ...
        st.metric(
            "Avg Degree", f"{avg_degree:.1f}", help="Average connections per node"
        )
...
for node in nodes:
    ...
    node_trace_by_type[ntype]["size"].append(
        min(node.get("size", 5) * 2, 30)
    )  # Scale size
...
with col1:
    ...
    entity_nodes = [
        (nid, data) for nid, data in sorted_nodes if data["type"] == "entity"
    ][:10]
...
with col2:
    ...
    insight_nodes = [
        (nid, data) for nid, data in sorted_nodes if data["type"] == "insight"
    ][:10]
...
with col3:
    ...
    video_nodes = [
        (nid, data) for nid, data in sorted_nodes if data["type"] == "video"
    ][:10]
    ...
    try:
        if nx.is_connected(G):
            ...
            st.metric(
                "Avg Clustering",
                f"{avg_clustering:.3f}",
                help="How tightly nodes cluster together",
            )
        else:
...
with col3:
    ...
    st.metric(
        "Connected Components", n_components, help="Number of separate sub-graphs"
    )
